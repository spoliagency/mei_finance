...
const CATEGORIAS_PF_DEFAULT = [
  { label: "Moradia", icon: "🏠" }, { label: "Alimentação", icon: "🛒" },
  { label: "Transporte", icon: "🚗" }, { label: "Saúde", icon: "💊" },
  { label: "Lazer", icon: "🎉" }, { label: "Assinaturas", icon: "🔄" },
  { label: "Vestuário", icon: "👗" }, { label: "Educação PF", icon: "🎓" },
  { label: "Pessoal", icon: "👤" }, { label: "Outro PF", icon: "📦" },
];
...
const CAT_COLORS = {
  ...
  "Moradia":"#6366f1","Alimentação":"#f59e0b","Transporte":"#3b82f6","Saúde":"#ec4899",
  "Lazer":"#10b981","Assinaturas":"#a855f7","Vestuário":"#f97316","Educação PF":"#06b6d4",
  "Pessoal":"#64748b","Outro PF":"#94a3b8",
};
const STATUS_STYLE = {
...
const IconClose = () => <svg width="18" height="18" fill="none" stroke="#999" strokeWidth="2" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>;

// ─── Shared CSS ───────────────────────────────────────────────────────────────
const CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  .input { background:#fff; border:1.5px solid #e0ddd8; border-radius:10px; padding:10px 14px; color:#1a1a1a; font-family:'Syne',sans-serif; font-size:13px; outline:none; width:100%; transition:border 0.15s; }
  ...
  .btn-dark { background:#1a1a1a; color:#f5f2ed; }
  .btn-dark:hover { background:#333; }
  .btn-outline { background:transparent; border:1.5px solid #d0cdc8; color:#666; }
  ...
  .btn-green:hover { background:#064e22; }
  .btn-icon { background:none; border:none; cursor:pointer; padding:6px; border-radius:8px; transition:background 0.15s; display:flex; align-items:center; justify-content:center; color:#888; }
  .btn-icon:hover { background:#e8e5e0; }
  .card { background:#fff; border-radius:16px; border:1px solid #e8e5e0; }
  ...
  .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.38); backdrop-filter:blur(6px); z-index:50; display:flex; align-items:center; justify-content:center; padding:20px; }
  .modal { background:#fff; border-radius:20px; width:100%; max-width:520px; max-height:90vh; overflow-y:auto; box-shadow:0 24px 80px rgba(0,0,0,0.18); }
  .divider { height:1px; background:#e8e5e0; }
  .filter-btn { padding:7px 13px; border-radius:8px; border:1.5px solid; cursor:pointer; font-size:12px; font-weight:600; font-family:'Syne',sans-serif; transition:all 0.15s; background:transparent; }
    ...
    <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
      {presets.map(([k,l]) => (
        <button key={k} className="filter-btn" onClick={() => pick(k)}
          style={{ borderColor: active===k ? "#1a1a1a":"#d0cdc8", background: active===k ? "#1a1a1a":"transparent", color: active===k ? "#fff":"#888" }}>
          {l}
        </button>
      ))}
      {active === "custom" && (
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:6 }}>
      {cats.map(c => (
        <button key={c.label} onClick={() => onChange(c.label)}
          style={{ padding:"8px 4px", borderRadius:10, border:"1.5px solid", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontSize:11, fontWeight:600, transition:"all 0.15s", textAlign:"center",
            background: value===c.label ? "#f5f2ed":"transparent",
            borderColor: value===c.label ? (CAT_COLORS[c.label]||"#888"):"#e0ddd8",
            color: value===c.label ? (CAT_COLORS[c.label]||"#888"):"#aaa" }}>
...
function CatGridEditable({ cats, value, onChange, onCatsChange }) {
  ...
  return (
    <div>
        ...
        {cats.map((c,i) => (
          <div key={c.label} style={{ position:"relative" }}>
            <button onClick={() => !editMode && onChange(c.label)}
              style={{ width:"100%", padding:"8px 4px", borderRadius:10, border:"1.5px solid", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontSize:11, fontWeight:600, transition:"all 0.15s", textAlign:"center",
      ...
      {showForm && (
        <div style={{ marginTop:12, background:"#f5f2ed", borderRadius:12, padding:"14px 16px", display:"flex", flexDirection:"column", gap:10 }}>
          <div style={{ fontSize:12, fontWeight:700, color:"#666" }}>{editingIdx===null?"Nova categoria":"Editar categoria"}</div>
          <input className="input" placeholder="Nome da categoria" value={form.label} onChange={e=>setForm(f=>({...f,label:e.target.value}))} style={{ background:"#fff" }} />
          <div>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={()=>setShowForm(false)} style={{ flex:1, padding:"8px", borderRadius:8, border:"1.5px solid #d0cdc8", background:"transparent", cursor:"pointer", fontSize:12, fontWeight:600, fontFamily:"'Syne',sans-serif", color:"#666" }}>Cancelar</button>
            <button onClick={saveForm} style={{ flex:2, padding:"8px", borderRadius:8, border:"none", background:"#1a1a1a", cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:"'Syne',sans-serif", color:"#f5f2ed" }}>Salvar</button>
          </div>
        </div>
      )}
    </div>
...
function RecordTable({ records, columns, onView, onEdit, onDelete, emptyMsg }) {
  return (
    <div className="card" style={{ overflow:"hidden" }}>
      {/* Header */}
      <div style={{ display:"grid", gridTemplateColumns: columns.map(c => c.w).join(" ") + " 88px", padding:"10px 20px", background:"#faf9f7", borderBottom:"1px solid #e8e5e0", alignItems:"center" }}>
        {[...columns.map(c => c.label), ""].map((h,i) => (
          <div key={i} style={{ fontSize:10, fontWeight:700, color:"#bbb", textTransform:"uppercase", letterSpacing:"0.7px", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{h}</div>
        ))}
      </div>
      {records.length === 0 && <div style={{ textAlign:"center", padding:"48px 0", color:"#ccc", fontSize:13 }}>{emptyMsg}</div>}
      {records.map((r, idx) => (
        <div key={r.id} className="row-hover" style={{ display:"grid", gridTemplateColumns: columns.map(c => c.w).join(" ") + " 88px", padding:"0 20px", minHeight:56, borderBottom: idx < records.length-1 ? "1px solid #f0ede8":"none", alignItems:"center", transition:"background 0.1s" }}>
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        <div style={{ fontSize:11, color:"#aaa", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.5px" }}>{cats.length} categorias</div>
        <button onClick={openAdd} style={{ background:"#1a1a1a", color:"#f5f2ed", border:"none", borderRadius:8, padding:"6px 13px", fontSize:12, fontWeight:700, fontFamily:"'Syne',sans-serif", cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}>
          <span style={{ fontSize:14 }}>+</span> Nova
        </button>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:5, marginBottom: showForm ? 14 : 0 }}>
        {cats.map((c, i) => (
          <div key={c.label} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 12px", borderRadius:10, background:"#faf9f7", border:"1px solid #f0ede8" }}>
      ...
      {showForm && (
        <div style={{ background:"#f5f2ed", borderRadius:12, padding:"16px", display:"flex", flexDirection:"column", gap:11, marginTop:4 }}>
          <div style={{ fontSize:12, fontWeight:700, color:"#555" }}>{editingIdx===null?"Nova categoria":"Editar categoria"}</div>
          <input className="input" placeholder="Nome da categoria" value={form.label} onChange={e=>setForm(f=>({...f,label:e.target.value}))} style={{ background:"#fff" }} />
          <div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 12px", borderRadius:9, background:"#fff", border:"1px solid #e8e5e0" }}>
      ...
      {cats.map(c => {
        ...
        return (
          <div key={c.label} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 12px", borderRadius:10, background:"#faf9f7", border:"1px solid #f0ede8" }}>
            ...
            <div style={{ fontSize:12, fontWeight:600, flex:1, minWidth:0, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{c.label}</div>
            {isEditing ? (
              <input autoFocus type="number" value={orcInput} onChange={e=>setOrcInput(e.target.value)}
                ...
                style={{ width:90, background:"#fff", border:"1.5px solid #1a1a1a", borderRadius:8, padding:"4px 10px", fontSize:12, fontWeight:700, fontFamily:"'JetBrains Mono',monospace", outline:"none" }} placeholder="R$ 0" />
            ) : (
              <button onClick={()=>{ setEditingOrc(c.label); setOrcInput(hasOrc?String(orc):""); }}
                style={{ background:hasOrc?"#f5f2ed":"transparent", border:hasOrc?"1px solid #e0ddd8":"1.5px dashed #d0cdc8", borderRadius:8, padding:"4px 12px", cursor:"pointer", fontFamily:"'JetBrains Mono',monospace", fontSize:12, fontWeight:700, color:hasOrc?"#1a1a1a":"#bbb", transition:"all 0.15s", flexShrink:0 }}>
...
function ConfigPage({ categoriasPJ, setCategoriasPJ, categoriasPF, setCategoriasPF, orcamentos, setOrcamento, perfil, setPerfil }) {
  ...
  const handlePhoto = (e) => {
    ...
    reader.onload = (ev) => setPerfil(p => ({ ...p, foto: ev.target.result }));
    reader.readAsDataURL(file);
  };

  const SIDEBAR_ITEMS = [
    { key:"perfil",    icon:"👤", label:"Perfil" },
    { key:"cat-pj",   icon:"🏢", label:"Categorias PJ" },
    { key:"cat-pf",   icon:"👤", label:"Categorias PF" },
    { key:"orc-pj",   icon:"📊", label:"Orçamentos PJ" },
    { key:"orc-pf",   icon:"📊", label:"Orçamentos PF" },
    { key:"prefs",    icon:"⚙️",  label:"Preferências" },
  ];

  return (
    <div style={{ display:"flex", gap:0, minHeight:600, background:"#fff", borderRadius:18, border:"1px solid #e8e5e0", overflow:"hidden" }}>

      {/* ── Sidebar ── */}
      <div style={{ width:220, background:"#faf9f7", borderRight:"1px solid #e8e5e0", padding:"24px 0", display:"flex", flexDirection:"column", flexShrink:0 }}>
        <div style={{ padding:"0 20px 20px", borderBottom:"1px solid #f0ede8", marginBottom:8 }}>
          <div style={{ fontSize:13, fontWeight:800, letterSpacing:"-0.3px" }}>Configurações</div>
          <div style={{ fontSize:11, color:"#aaa", marginTop:2 }}>freela.fin · MEI</div>
        </div>
        <nav style={{ flex:1, padding:"0 10px" }}>
          {SIDEBAR_ITEMS.map(item => (
            <button key={item.key} onClick={() => setSection(item.key)}
              style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"9px 12px", borderRadius:9, border:"none", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:600, textAlign:"left", transition:"all 0.15s", marginBottom:2,
                background: section===item.key ? "#1a1a1a" : "transparent",
                color: section===item.key ? "#f5f2ed" : "#666" }}>
        ...
        <div style={{ padding:"16px 16px 0", borderTop:"1px solid #f0ede8", marginTop:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:34, height:34, borderRadius:99, background:"#e8e5e0", overflow:"hidden", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:15 }}>
              {perfil.foto ? <img src={perfil.foto} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : "👤"}
            </div>
            <div style={{ minWidth:0 }}>
              <div style={{ fontSize:12, fontWeight:700, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{perfil.nome || "Seu nome"}</div>
              <div style={{ fontSize:10, color:"#aaa" }}>{perfil.tipo || "MEI"}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex:1, padding:"32px 36px", overflowY:"auto" }}>
        ...
        {section === "perfil" && (
          <div>
            ...
            {/* Photo */}
            <div style={{ display:"flex", alignItems:"center", gap:20, marginBottom:28, padding:"20px", background:"#f5f2ed", borderRadius:14 }}>
              <div style={{ position:"relative", flexShrink:0 }}>
                <div style={{ width:72, height:72, borderRadius:99, background:"#e0ddd8", overflow:"hidden", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, border:"3px solid #fff" }}>
                  {perfil.foto ? <img src={perfil.foto} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : "👤"}
                </div>
                {perfil.foto && (
                  <button onClick={() => setPerfil(p=>({...p,foto:null}))}
                    style={{ position:"absolute", top:-4, right:-4, width:20, height:20, borderRadius:99, background:"#ef4444", border:"2px solid #fff", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <svg width="9" height="9" fill="none" stroke="#fff" strokeWidth="2.5" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                  </button>
                )}
              </div>
              <div>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <label style={{ background:"#1a1a1a", color:"#f5f2ed", borderRadius:8, padding:"7px 14px", fontSize:12, fontWeight:700, fontFamily:"'Syne',sans-serif", cursor:"pointer", display:"inline-flex", alignItems:"center", gap:5 }}>
                    📷 {perfil.foto ? "Alterar foto" : "Adicionar foto"}
                    <input type="file" accept="image/*" style={{ display:"none" }} onChange={handlePhoto} />
                  </label>
                  {perfil.foto && (
                    <button onClick={() => setPerfil(p=>({...p,foto:null}))} style={{ background:"transparent", border:"1.5px solid #d0cdc8", borderRadius:8, padding:"7px 14px", fontSize:12, fontWeight:600, fontFamily:"'Syne',sans-serif", cursor:"pointer", color:"#888" }}>
                      Remover
                    </button>
                  )}
                </div>
                <div style={{ fontSize:11, color:"#bbb", marginTop:6 }}>JPG, PNG ou GIF · máx. 5MB</div>
              </div>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>Nome completo</label>
                  <input className="input" placeholder="Seu nome" value={perfil.nome||""} onChange={e=>setPerfil(p=>({...p,nome:e.target.value}))} />
                </div>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>Como prefere ser chamado</label>
                  <input className="input" placeholder="Apelido ou nome curto" value={perfil.apelido||""} onChange={e=>setPerfil(p=>({...p,apelido:e.target.value}))} />
                </div>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>Tipo de MEI / Atividade</label>
                  <select className="input" value={perfil.tipo||""} onChange={e=>setPerfil(p=>({...p,tipo:e.target.value}))}>
                ...
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>Profissão / Ocupação</label>
                  <input className="input" placeholder="Ex: Consultor de vendas" value={perfil.profissao||""} onChange={e=>setPerfil(p=>({...p,profissao:e.target.value}))} />
                </div>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>CNPJ</label>
                  <input className="input" placeholder="00.000.000/0001-00" value={perfil.cnpj||""} onChange={e=>setPerfil(p=>({...p,cnpj:e.target.value}))} />
                </div>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>CPF</label>
                  <input className="input" placeholder="000.000.000-00" value={perfil.cpf||""} onChange={e=>setPerfil(p=>({...p,cpf:e.target.value}))} />
                </div>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>E-mail</label>
                  <input className="input" type="email" placeholder="seu@email.com" value={perfil.email||""} onChange={e=>setPerfil(p=>({...p,email:e.target.value}))} />
                </div>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>Telefone / WhatsApp</label>
                  <input className="input" placeholder="(11) 99999-9999" value={perfil.tel||""} onChange={e=>setPerfil(p=>({...p,tel:e.target.value}))} />
                </div>
              </div>
              <div>
                <label style={{ fontSize:11, fontWeight:700, color:"#aaa", textTransform:"uppercase", letterSpacing:"0.6px", display:"block", marginBottom:6 }}>Nome fantasia / Empresa</label>
                <input className="input" placeholder="Ex: João Silva Consultoria" value={perfil.empresa||""} onChange={e=>setPerfil(p=>({...p,empresa:e.target.value}))} />
              </div>
              <div style={{ display:"flex", justifyContent:"flex-end", paddingTop:4 }}>
                <div style={{ fontSize:11, color:"#16a34a", fontWeight:600, background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:7, padding:"5px 12px" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {[
                ...
                <div key={f.key} className="card" style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:16 }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:11, color:"#aaa" }}>{f.sub}</div>
                  </div>
                  <input className="input" type={f.type} placeholder={f.placeholder} style={{ maxWidth:140, textAlign:"right", fontFamily:"'JetBrains Mono',monospace", fontWeight:700 }}
                    value={perfil[f.key]||""} onChange={e=>setPerfil(p=>({...p,[f.key]:e.target.value}))} />
                </div>
              ))}
              ...
              <div className="card" style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:16 }}>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:"#aaa" }}>Afeta o valor do DAS calculado automaticamente</div>
                </div>
                <select className="input" style={{ maxWidth:180 }} value={perfil.tipo||""} onChange={e=>setPerfil(p=>({...p,tipo:e.target.value}))}>
                  <option value="">Selecionar...</option>
                  <option>Serviços</option>
                  <option>Comércio</option>
                  <option>Indústria</option>
...
export default function App() {
  ...
  const [view, setView] = useState("lista");    // "lista" | "categorias"
  const [modal, setModal] = useState(null);     // { type, mode, record? }
  const [deleteConfirm, setDeleteConfirm] = useState(null);

  // Data
  const [vendas, setVendas] = useState([
    { id:1, descricao:"Proteção Veicular Premium", cliente:"João Silva", metodo:"PIX", faturamento:850, taxas:[{label:"Taxa plataforma",pct:5},{label:"Repasse",pct:10}], data:"2024-01-15", status:"recebido", obs:"" },
    { id:2, descricao:"Plano Básico", cliente:"Maria Santos", metodo:"Cartão Crédito", faturamento:520, taxas:[{label:"Taxa plataforma",pct:5},{label:"Taxa cartão",pct:2.5}], data:"2024-01-18", status:"recebido", obs:"Parcelado 3x" },
    { id:3, descricao:"Consultoria onboarding", cliente:"Pedro Rocha", metodo:"PIX", faturamento:300, taxas:[{label:"Taxa plataforma",pct:5}], data:"2024-01-20", status:"pendente", obs:"" },
  ]);
  ...
  // Perfil
  const [perfil, setPerfil] = useState({ nome:"", apelido:"", tipo:"Serviços", profissao:"", cnpj:"", cpf:"", email:"", tel:"", empresa:"", foto:null, diaFechamento:"20", prolabore:"", metaReceita:"", reservaEmerg:"6" });
  ...
  // ── Filter helpers ──
  const applyFilters = (records, valKey) => records.filter(r => {
  ...
  const totals = useMemo(() => {
    const vendasRec = vendas.filter(v=>v.status==="recebido");
    ...
    const totalDesp = despesas.filter(d=>d.status==="pago").reduce((s,d)=>s+d.valor,0);
    const totalGastos = gastos.filter(g=>g.status==="pago").reduce((s,g)=>s+g.valor,0);
    const pendentesVenda = vendas.filter(v=>v.status==="pendente").reduce((s,v)=>s+v.faturamento,0);
    const pendentesDespesa = despesas.filter(d=>d.status==="pendente").reduce((s,d)=>s+d.valor,0);
    const pendentesGasto = gastos.filter(g=>g.status==="pendente").reduce((s,g)=>s+g.valor,0);
    return { totalBruto, totalLiq, totalDesp, totalGastos, resultado: totalLiq-totalDesp, pendentesVenda, pendentesDespesa, pendentesGasto };
  }, [vendas, despesas, gastos]);

  // ── Category breakdown ──
  const catBreakdown = useMemo(() => {
    const src = isPJ ? despesas.filter(d=>d.status==="pago") : gastos.filter(g=>g.status==="pago");
    const map = {};
    src.forEach(r => { map[r.categoria] = (map[r.categoria]||0) + r.valor; });
    return Object.entries(map).sort((a,b)=>b[1]-a[1]);
  }, [isPJ, despesas, gastos]);
  ...
  const summaryCards = isPJ ? [
    { label:"Receita bruta",   value:fmt(totals.totalBruto), sub:`${vendas.filter(v=>v.status==="recebido").length} vendas recebidas`, accent:"#16a34a" },
    { label:"Valor líquido",   value:fmt(totals.totalLiq),   sub:"após taxas e repasses",   accent:"#4ade80" },
    { label:"Despesas PJ",     value:fmt(totals.totalDesp),  sub:"pagas no período",         accent:"#c0392b" },
    { label:"Resultado",       value:fmt(totals.resultado),  sub:"líquido − despesas",       accent: totals.resultado>=0?"#6366f1":"#ef4444" },
  ] : [
    { label:"Entradas PF",     value:fmt(totals.totalLiq),   sub:"pró-labore + outros",      accent:"#16a34a" },
    { label:"Gastos pessoais", value:fmt(totals.totalGastos),sub:"total pago",               accent:"#c0392b" },
    { label:"Saldo pessoal",   value:fmt(totals.totalLiq-totals.totalGastos), sub:"entradas − gastos", accent: (totals.totalLiq-totals.totalGastos)>=0?"#6366f1":"#ef4444" },
  ...
  const vendasCols = [
    { label:"Descrição / Cliente", w:"3fr", render: r => (
      <div>
        ...
        <div style={{fontSize:11,color:"#aaa"}}>{r.cliente} · {r.data}</div>
      </div>
    )},
    { label:"Método",  w:"1.4fr", render: r => <span style={{fontSize:12,color:"#666"}}>{r.metodo}</span> },
    ...
    { label:"Líquido", w:"1.2fr", render: r => {
      ...
      return <div style={{lineHeight:1.3}}>
        <div style={{fontSize:13,fontWeight:700,color:"#16a34a",fontFamily:"'JetBrains Mono',monospace",whiteSpace:"nowrap"}}>{fmt(c.liquido)}</div>
        <div style={{fontSize:10,color:"#bbb",marginTop:1}}>−{fmtPct(c.totalPct)}</div>
      </div>;
    }},
  ...
  const despesasCols = [
    { label:"Descrição", w:"2.5fr", render: r => (
      <div>
        ...
        <div style={{fontSize:11,color:"#aaa",display:"flex",alignItems:"center",gap:4}}>
          <span>{r.data}</span>
          {r.recorrencia!=="Único" && <span style={{background:"#f0ede8",borderRadius:4,padding:"1px 5px",fontSize:10,fontWeight:600,color:"#888"}}>{r.recorrencia}</span>}
        </div>
      </div>
    )},
    { label:"Categoria", w:"2fr", render: r => (
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        <span style={{fontSize:12,color:"#555"}}>{r.categoria}</span>
      </div>
    )},
  ...
  return (
    <div style={{ fontFamily:"'Syne',sans-serif", background:"#f5f2ed", minHeight:"100vh", color:"#1a1a1a" }}>
        ...
        <div style={{ maxWidth:1080, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:64 }}>
          <div style={{ display:"flex", alignItems:"center", gap:16 }}>
            <div>
              ...
              <div style={{ fontSize:11, color:"#555", fontFamily:"'JetBrains Mono',monospace", marginTop:1 }}>{perfil.nome ? perfil.nome : "MEI · controle financeiro"}</div>
            </div>
            {/* Context switcher */}
            <div style={{ display:"flex", background:"#2a2a2a", borderRadius:10, padding:3, marginLeft:8 }}>
              {[["pj","🏢 Empresa"],["pf","👤 Pessoal"]].map(([k,l])=>(
                <button key={k} onClick={()=>{ setContext(k); setSearch(""); setFilterStatus("todos"); }}
                  style={{ width:108, padding:"7px 0", borderRadius:8, border:"none", cursor:"pointer", fontSize:13, fontWeight:700, fontFamily:"'Syne',sans-serif", transition:"all 0.15s", textAlign:"center",
                    background: context===k ? "#f5f2ed":"transparent", color: context===k ? "#1a1a1a":"#555" }}>{l}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", flexShrink:0 }}>
            ...
            <button
              ...
              style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, width:148, height:38, fontSize:13, fontWeight:700, fontFamily:"'Syne',sans-serif", borderRadius:10, border:"none", cursor:"pointer", transition:"all 0.15s", flexShrink:0,
            ...
            <button
              style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, width:136, height:38, fontSize:13, fontWeight:700, fontFamily:"'Syne',sans-serif", borderRadius:10, border:"none", cursor:"pointer", transition:"all 0.15s", flexShrink:0,
                background: page === "main" ? "#f5f2ed" : "transparent",
                color: page === "main" ? "#1a1a1a" : "transparent",
            ...
            {/* Slot 3: gear icon — always visible */}
            <button onClick={() => setPage(p => p === "config" ? "main" : "config")}
              title="Configurações"
              style={{ width:38, height:38, borderRadius:10, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.15s", flexShrink:0,
                background: page === "config" ? "#f5f2ed" : "#2a2a2a", color: page === "config" ? "#1a1a1a" : "#888" }}>
      ...
      <div style={{ maxWidth:1080, margin:"0 auto", padding:"28px 32px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:12 }}>
          <div style={{ display:"flex", background:"#e8e5e0", borderRadius:10, padding:3 }}>
            {[["lista","Lista"],["categorias","Por categoria"]].map(([k,l])=>(
              <button key={k} onClick={()=>setView(k)}
                style={{ padding:"7px 16px", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:600, fontFamily:"'Syne',sans-serif", transition:"all 0.15s",
                  background:view===k?"#fff":"transparent", color:view===k?"#1a1a1a":"#888", boxShadow:view===k?"0 1px 4px rgba(0,0,0,0.08)":"none" }}>{l}</button>
            ))}
          </div>
          ...
          <div style={{ display:"flex", gap:6 }}>
            {["todos","recebido","pago","pendente","cancelado"].filter(s => isPJ ? s!=="pago" : s!=="recebido").map(s=>(
              <button key={s} className="filter-btn" onClick={()=>setFilterStatus(s)}
                style={{ borderColor:filterStatus===s?"#1a1a1a":"#d0cdc8", background:filterStatus===s?"#1a1a1a":"transparent", color:filterStatus===s?"#fff":"#888" }}>
            ...
            <div className="modal">
              ...
              <div style={{ padding:"24px 28px" }}>
                ...
                <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:20 }}>
                  ...
                  <span style={{ fontSize:12, color:"#888" }}>{r.data}</span>
                  <span style={{ color:"#ccc" }}>·</span>
              ...
              <div style={{ padding:"24px 28px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:"#aaa", marginTop:2 }}>{r.categoria} · {r.data}</div>
                  </div>
                </div>
